 ...
 * @version 0.0.3
 */
function f(){function appendAndRemove(url,name,ext){const utime=(new Date).getTime();const fullExt=ext?`.${ext}`:"";const a=document.createElement("a");a.target="_blank";a.rel="noopener";a.href=url;a.download=name?name:`${utime}${fullExt}`;document.body.appendChild(a);a.click();setTimeout((()=>{a.remove()}),250)}function get(url){return fetch(url).then((r=>{const type=r.headers.get("content-type").split(";")[0].trim();return new Promise((resolve=>{r.blob().then((blob=>resolve({blob:blob,type:type})))}))}))}function getUrlFor(blob){const url=URL.createObjectURL(blob);setTimeout((()=>{URL.revokeObjectURL(url)}),1e4);return url}
/**
